use crate::avian_box_container_pile_10k_case as avian_case;
    ...
    I: Iterator<Item = String>,
{
    ...
    initialize_worker_pools(cli.thread_count)?;
    ...
    case_registry::step_world(descriptor, &mut world, warmup_steps);
    send_completed(&mut stream, FRAME_COMPLETED, 3, STATUS_OK, 0, 0.0)?;

    let mut completed_steps = 0usize;
    let mut physics_elapsed_ms = 0.0f64;
    let mut transforms = vec![avian_case::AvianTransform { position_x: 0.0, position_y: 0.0, position_z: 0.0, rotation_x: 0.0, rotation_y: 0.0, rotation_z: 0.0, rotation_w: 1.0 }; descriptor.dynamic_body_count];
    let mut boxes = vec![avian_case::AvianStaticBox { position_x: 0.0, position_y: 0.0, position_z: 0.0, half_extent_x: 0.0, half_extent_y: 0.0, half_extent_z: 0.0 }; descriptor.static_body_count];
...
pub fn initialize_worker_pools(thread_count: usize) -> Result<(), u8> {
    ...
    if ComputeTaskPool::get().thread_num() != thread_count || rayon::current_num_threads() != thread_count {
...
pub fn send_hello_ack(stream: &mut TcpStream, cli: &SharedVisualCli, descriptor: case_registry::AvianCaseDescriptor, sequence: u32) -> Result<(), u8> {
    ...
    write_text(&mut payload, "avian3d_polygon_runner");
...
pub fn build_snapshot_payload(
    descriptor: case_registry::AvianCaseDescriptor,
    config: &CaseRunConfig,
    world: &avian_case::AvianWorld,
    completed_steps: usize,
    physics_elapsed_ms: f64,
    transforms: &mut [avian_case::AvianTransform],
    boxes: &mut [avian_case::AvianStaticBox],
    payload: &mut Vec<u8>,
) -> Result<(), u8> {
    avian_case::sample_transforms(world, transforms).map_err(|_| {
        eprintln!("run_failed reason=sample_transforms");
        2u8
    })?;
    avian_case::copy_static_boxes(boxes).map_err(|_| {
