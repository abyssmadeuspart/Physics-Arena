...
int ParseArgs(int argc, char** argv, SharedVisualCli* cli)
{
	...
	if (std::strcmp(cli->producerMode, "shared-visual") != 0 || std::strcmp(cli->connectHost, "127.0.0.1") != 0 ||
		std::strcmp(cli->engineId, jolt_benchmark::kEngineId) != 0 || cli->caseId[0] == '\0' || cli->runToken[0] == '\0' ||
		cli->connectPort <= 0)
	{
...
int SendHelloAck(VisualSocket socket, const SharedVisualCli& cli, const jolt_benchmark::JoltCaseDescriptor& descriptor, uint32_t sequence)
{
	benchmark_visual::VisualHelloAckPayload ack = {};
	CopyText(ack.engineId, jolt_benchmark::kEngineId);
	CopyText(ack.caseId, descriptor.caseId);
	CopyText(ack.producerBuildId, "jolt_polygon_runner");
#if defined(_WIN32)
...
int DecodeStart(const SharedVisualCli& cli, const std::vector<uint8_t>& payload, uint32_t payloadBytes, jolt_benchmark::JoltCaseConfig* config)
{
...
void FillIdentity(const jolt_benchmark::JoltCaseDescriptor& descriptor, const jolt_benchmark::JoltCaseConfig& config, benchmark_visual::VisualIdentity* identity)
{
	CopyText(identity->caseId, descriptor.caseId);
	CopyText(identity->engineId, jolt_benchmark::kEngineId);
...
uint32_t BuildSnapshotPayload(
	const jolt_benchmark::JoltCaseDescriptor& descriptor,
	const jolt_benchmark::JoltCaseState& state,
	jolt_benchmark::JoltTransform* caseTransforms,
	jolt_benchmark::JoltStaticBox* caseBoxes,
...
int RunSharedVisualLoop(const SharedVisualCli& cli)
{
	...
	if (jolt_benchmark::ResolveJoltCase(cli.caseId, &descriptor) != 0)
	{
	...
	int exitCode = 2;
	int runtimeStarted = 0;
	int socketSystemStarted = 0;
	int stateCreated = 0;
	VisualSocket socket = kInvalidVisualSocket;
	jolt_benchmark::JoltCaseConfig config = {};
	jolt_benchmark::JoltCaseState state = {};
	std::vector<uint8_t> payload(benchmark_visual::kMaxFramePayloadBytes);
	std::vector<jolt_benchmark::JoltTransform> caseTransforms(descriptor->dynamicBodyCount);
	std::vector<jolt_benchmark::JoltStaticBox> caseBoxes(descriptor->staticBodyCount);
