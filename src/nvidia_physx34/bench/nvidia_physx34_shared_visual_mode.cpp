...
int ParseArgs(int argc, char** argv, SharedVisualCli* cli)
{
	...
	if (std::strcmp(cli->producerMode, "shared-visual") != 0 || std::strcmp(cli->connectHost, "127.0.0.1") != 0 ||
		std::strcmp(cli->engineId, nvidia_physx34_benchmark::kEngineId) != 0 || cli->caseId[0] == '\0' || cli->runToken[0] == '\0' ||
		cli->connectPort <= 0)
	{
...
int SendHelloAck(VisualSocket socket, const SharedVisualCli& cli, const nvidia_physx34_benchmark::PhysXCaseDescriptor& descriptor, uint32_t sequence)
{
	benchmark_visual::VisualHelloAckPayload ack = {};
	CopyText(ack.engineId, nvidia_physx34_benchmark::kEngineId);
	CopyText(ack.caseId, descriptor.caseId);
	CopyText(ack.producerBuildId, "nvidia_physx34_polygon_runner");
#if defined(_WIN32)
...
int DecodeStart(const SharedVisualCli& cli, const std::vector<uint8_t>& payload, uint32_t payloadBytes, nvidia_physx34_benchmark::PhysXCaseConfig* config)
{
...
void FillIdentity(const nvidia_physx34_benchmark::PhysXCaseDescriptor& descriptor, const nvidia_physx34_benchmark::PhysXCaseConfig& config, benchmark_visual::VisualIdentity* identity)
{
	CopyText(identity->caseId, descriptor.caseId);
	CopyText(identity->engineId, nvidia_physx34_benchmark::kEngineId);
...
uint32_t BuildSnapshotPayload(
	const nvidia_physx34_benchmark::PhysXCaseDescriptor& descriptor,
	const nvidia_physx34_benchmark::PhysXCaseState& state,
	nvidia_physx34_benchmark::PhysXTransform* caseTransforms,
	nvidia_physx34_benchmark::PhysXStaticBox* caseBoxes,
...
int RunSharedVisualLoop(const SharedVisualCli& cli)
{
	...
	if (nvidia_physx34_benchmark::ResolvePhysXCase(cli.caseId, &descriptor) != 0)
	{
	...
	nvidia_physx34_benchmark::PhysXCaseConfig config = {};
	nvidia_physx34_benchmark::PhysXCaseState state = {};
	std::vector<uint8_t> payload(benchmark_visual::kMaxFramePayloadBytes);
	std::vector<nvidia_physx34_benchmark::PhysXTransform> caseTransforms(descriptor->dynamicBodyCount);
	std::vector<nvidia_physx34_benchmark::PhysXStaticBox> caseBoxes(descriptor->staticBodyCount);
