...
int AddStaticBox(PhysXContext* context, physx::PxVec3 position, float hx, float hy, float hz)
{
	...
	physx::PxShape* shape = physx::PxRigidActorExt::createExclusiveShape(*actor, physx::PxBoxGeometry(hx, hy, hz), *context->material);
	if (shape == nullptr)
	{
		actor->release();
...
int AddDynamicBox(PhysXContext* context, physx::PxRigidDynamic** bodies, int bodyIndex, physx::PxVec3 position)
{
	...
	physx::PxShape* shape = physx::PxRigidActorExt::createExclusiveShape(*actor, physx::PxBoxGeometry(kHalfExtent, kHalfExtent, kHalfExtent), *context->material);
	if (shape == nullptr)
	{
		actor->release();
